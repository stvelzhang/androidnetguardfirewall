        ...
        if (isFirewallOn) {
            ...
            for (InterceptRules rule: rules) {
                ...
                if (rule.getSession().equals("N/A")) {
                    ...
                    String realSessionIP =  chain.request().ip();//chain.request().host();//
                    int realSessionPort = chain.request().port();
                    Log.w(TAG, "realSessionIP:" + realSessionIP + "--realSessionPort:" + realSessionPort);
                    Log.w(TAG, "ruleIPandPort[0]:" + ruleIPandPort[0]);
